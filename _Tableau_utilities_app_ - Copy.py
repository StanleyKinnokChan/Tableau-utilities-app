...
######### Language Pannel for translation button
def open_language_selection():
    ...
    text_translation_warning = tk.Label(language_window, text="Noted that it only translates the text in the containers and titles on dashboards\nIt didn't translate the text putted in the sheet or the data itself.\n\nIt can takes couples minutes if the dashboard is large. Please don't click the app during waiting.", font=("Arial", 9), anchor="w")  # Align text to the left
    ...
    def confirm_selection():
        ...
        if file.endswith('.twbx'):
            ...
            old_folder =  os.path.join(dir, 'unzipped_twbx')
            base_name = os.path.basename(file).split('.')[0]
            new_folder = os.path.join(dir, f'{base_name}_translated')
            os.rename(old_folder,new_folder)

            #read .twb inside the unzip folder
            twb_file_path = os.path.join(new_folder, f'{base_name}.twb')
